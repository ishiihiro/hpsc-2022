...
int main(int argc, char** argv) {
  ...
  Body ibody[N/size], jbody[N/size], rcv[N/size];
  srand48(rank);
  ...
  MPI_Win win;
  MPI_Win_create(rcv, N*sizeof(Body)/size, sizeof(Body), MPI_INFO_NULL, MPI_COMM_WORLD, &win);
  // Can't use jbody as window. I observed this program giving wrong output when jbody was used as window
  // especially after this program had been executed multiple times.
  for(int irank=0; irank<size; irank++) {
    MPI_Win_fence(0, win);
    MPI_Put(jbody, N/size, MPI_BODY, send_to, 0, N/size, MPI_BODY, win);
    MPI_Win_fence(0, win);
    for(int i=0; i<N/size; i++) jbody[i] = rcv[i];
    for(int i=0; i<N/size; i++) {
